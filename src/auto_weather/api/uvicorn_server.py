...
def run_server(host: str = "127.0.0.1", port: int = 8000, reload: bool = False, log_level: str = "info", uvicorn_log_fmt: str = "%(asctime)s - %(levelname)s - %(message)s"):
    ...
    try:
        uvicorn.run("auto_weather.api.api_main:api_app", host=host, port=port, reload=reload, log_config=log_config, log_level=log_level)
