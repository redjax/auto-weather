...
from auto_weather.core.db import Base
...
from auto_weather.core.depends.db_depends import get_db_engine
...
if engine.url:
    try:
        ...
        msg = f"({type(exc)}) Error setting sqlalchemy.url setting for Alembic."
        print(f"[ERROR] {msg}. Trying again using urllib.parse.unquote")

        config.set_main_option(
            "sqlalchemy.url", unquote(engine.url.render_as_string(hide_password=False))
        )
